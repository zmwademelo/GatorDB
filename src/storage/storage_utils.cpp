...
std::vector<char> serialize(TableMetadata& metadata) {//
    ...
    result.insert(result.end(), reinterpret_cast<char*>(&name_length), reinterpret_cast<char*>(&name_length) + sizeof(uint32_t));
    result.insert(result.end(), metadata.table_name.begin(), metadata.table_name.end());
    
    // Serialize first_page_id
    result.insert(result.end(), reinterpret_cast<char*>(&metadata.first_page_id), reinterpret_cast<char*>(&metadata.first_page_id) + sizeof(page_id_t));
    
    // Serialize schema - number of columns
    const auto& columns = metadata.schema.get_columns();
    uint32_t num_columns = columns.size();
    result.insert(result.end(), reinterpret_cast<char*>(&num_columns), reinterpret_cast<char*>(&num_columns) + sizeof(uint32_t));
    
    // Serialize each column
    for (const auto& col : columns) {
        // Column name (length-prefixed)
        uint32_t col_name_length = col.name.length();
        result.insert(result.end(), reinterpret_cast<char*>(&col_name_length), reinterpret_cast<char*>(&col_name_length) + sizeof(uint32_t));
        ...
        result.insert(result.end(), reinterpret_cast<char*>(&type_val), reinterpret_cast<char*>(&type_val) + sizeof(uint32_t));
        // Column length
        uint32_t col_length = col.length;
        result.insert(result.end(), reinterpret_cast<char*>(&col_length), reinterpret_cast<char*>(&col_length) + sizeof(uint32_t));
    }
...
TableMetadata deserialize(const std::vector<char>& buffer) {
    ...
    uint32_t name_length = *reinterpret_cast<const uint32_t*>(buffer.data() + offset); //length can be negative
    offset += sizeof(uint32_t);
    std::string table_name(buffer.data() + offset, buffer.data() + offset + name_length);
    offset += name_length;
    
    // Deserialize first_page_id
    page_id_t first_page_id = *reinterpret_cast<const page_id_t*>(buffer.data() + offset);
    offset += sizeof(page_id_t);
    
    // Deserialize schema - number of columns
    uint32_t num_columns = *reinterpret_cast<const uint32_t*>(buffer.data() + offset);
    offset += sizeof(uint32_t);
    ...
    for (uint32_t i = 0; i < num_columns; ++i) {
        Column col;
        // Column name
        uint32_t col_name_length = *reinterpret_cast<const uint32_t*>(buffer.data() + offset);
        offset += sizeof(uint32_t);
        col.name = std::string(buffer.data() + offset, buffer.data() + offset + col_name_length);
        offset += col_name_length;
        // Column type
        uint32_t type_val = *reinterpret_cast<const uint32_t*>(buffer.data() + offset);
        offset += sizeof(uint32_t);
...
Value Value::deserialize(const char* buffer, Type type) {
    if (type == Type::INTEGER) {
        ...
        std::string str(buffer); //Assumes the buffer is null-terminated. In our case, since we always clear the bytes before writing a new string, this should be safe because any unused bytes will be nulls.
        return Value(str); 
    } else {
...
TableMetadata getTableMetadata(Pager& pager, std::string table_name) {
...
std::vector<std::vector<Value>> readRecords(Pager& pager, const std::string& table_name) {
    ...
    while (page_head != INVALID_PAGE_ID) {
        ...
        PageHeader& page_head_header = *reinterpret_cast<PageHeader*>(page_head_buffer.data()); 

        auto slot_count = page_head_header.slot_count; 

        for (auto slot_num = 0; slot_num < slot_count; ++slot_num) {
            std::vector<char> raw_bytes = pager.get_record_raw_bytes(page_head, slot_num); 
...
page_id_t insert_at_page_or_new(Pager& pager, page_id_t page_head, std::vector<char>& raw_bytes) {
    ...
    if (last_page_id != INVALID_PAGE_ID) {
        ...
        PageHeader& new_tail_page_header = *reinterpret_cast<PageHeader*>(new_tail_page_buffer.data());
        new_tail_page_header.prev_page_id = last_page_id; 
        
        pager.write_page(last_page_id, last_page_buffer.data()); // Don't forget to persist the change!
...
void peekPage(Pager& pager, page_id_t page_id) {
    ...
    if (page_id == 0){ //For File header
        ...
        std::cout <<"Catalog Page Head: " << file_header->catalog_page_head_ << std::endl; 
        std::cout <<"Catalog Page Tail: " << file_header->catalog_page_tail_ << std::endl; 
        std::cout <<"Page count: " << file_header->page_count_ << std::endl; 
        std::cout <<"Free Page Head: " << file_header->free_page_head_ << std::endl; 
        std::cout <<"Free Page Tail: " << file_header->free_page_tail_ << std::endl; 
        ...
        std::cout<< "------Page Data: ------" << std::endl; //No cache, cannot get table name
        for (auto slot_num = 0; slot_num < page_header.slot_count; ++slot_num) {
